...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embeddings = nn.Parameter(torch.tensor(embeddings)) # self.embeddings would be a module parameter
        ...
        self.embed2hid = nn.Linear(in_features=self.n_features*self.embed_size, out_features=self.hidden_size)
        self.relu = nn.ReLU(inplace=True)
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hid2out = nn.Linear(in_features=self.hidden_size, out_feature=self.n_classes)
        self.embed_to_hidden_weight = nn.Parameter(data=self.embed2hid.weight)
        self.embed_to_hidden_bias = nn.Parameter(data=self.embed2hid.bias)
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)
        self.hidden_to_logits_weight = nn.Parameter(data=self.hid2out.weight)
        self.hidden_to_logits_bias = nn.Parameter(data=self.hid2out.bias)
