...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ['ROOT']
        self.buffer = sentence[:] # list has no .copy() method, while dict has.
        self.dependencies = []
        ### END YOUR CODE
    ...
    def parse_step(self, transition):
        ...
        if transition=="S" and len(self.buffer)>0:
            ...
            self.dependencies.append((self.stack[0], self.stack[1]))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        mini_batch = unfinished_parses[:batch_size] if len(unfinished_parses)>batch_size else unfinished_parses
        next_transisions = model.predict(mini_batch)
        # print("next_transisions:", next_transisions)
        # print("unfinished_poarsers:", unfinished_parses)
        for idx, parser in enumerate(mini_batch):
            dep = parser.parse(next_transisions[idx])
            print('dep:', dep)
            dependencies.append(dep)
        unfinished_parses = unfinished_parses[batch_size:] if len(unfinished_parses)>batch_size else []
    ## END YOUR CODE
    return dependencies
